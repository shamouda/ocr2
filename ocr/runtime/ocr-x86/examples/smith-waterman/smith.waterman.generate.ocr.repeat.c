...
typedef struct {
    ...
    ocrGuid_t bottom_row_db_guid;
    ocrGuid_t right_column_db_guid;
    ocrGuid_t bottom_right_db_guid;
} Tile_t;
...
u8 smith_waterman_task ( u32 paramc, u64 * params, void* paramv[], u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrDbCreate( &tile_matrix[i][j].bottom_right_db_guid, &db_guid_i_j_br_data, sizeof(int), FLAGS, NULL, NO_ALLOC );
    ...
    ocrEventSatisfy(tile_matrix[i][j].bottom_right_event_guid, tile_matrix[i][j].bottom_right_db_guid);

    /* Allocate datablock for right column of the local tile */
    void* db_guid_i_j_rc_data;
    ocrDbCreate( &tile_matrix[i][j].right_column_db_guid, &db_guid_i_j_rc_data, sizeof(int)*tile_height, FLAGS, NULL, NO_ALLOC );
    ...
    ocrEventSatisfy(tile_matrix[i][j].right_column_event_guid, tile_matrix[i][j].right_column_db_guid);

    /* Allocate datablock for bottom row of the local tile */
    void* db_guid_i_j_brow_data;
    ocrDbCreate( &tile_matrix[i][j].bottom_row_db_guid, &db_guid_i_j_brow_data, sizeof(int)*tile_width, FLAGS, NULL, NO_ALLOC );
    ...
    ocrEventSatisfy(tile_matrix[i][j].bottom_row_event_guid, tile_matrix[i][j].bottom_row_db_guid);
...
static void initialize_border_values( Tile_t** tile_matrix, int n_tiles_width, int n_tiles_height, int tile_width, int tile_height ) {
    ...
    ocrDbCreate( &tile_matrix[0][0].bottom_right_db_guid, &db_guid_0_0_br_data, sizeof(int), FLAGS, NULL, NO_ALLOC );
    ...
    allocated[0] = 0;
    ocrEventSatisfy(tile_matrix[0][0].bottom_right_event_guid, tile_matrix[0][0].bottom_right_db_guid);
    ...
    for ( j = 1; j < n_tiles_width + 1; ++j ) {
        ...
        ocrDbCreate( &tile_matrix[0][j].bottom_row_db_guid, &db_guid_0_j_brow_data, sizeof(int)*tile_width, FLAGS, NULL, NO_ALLOC );
        ...
        ocrEventSatisfy(tile_matrix[0][j].bottom_row_event_guid, tile_matrix[0][j].bottom_row_db_guid);

        void* db_guid_0_j_br_data;
        ocrDbCreate( &tile_matrix[0][j].bottom_right_db_guid, &db_guid_0_j_br_data, sizeof(int), FLAGS, NULL, NO_ALLOC );
        ...
        ocrEventSatisfy(tile_matrix[0][j].bottom_right_event_guid, tile_matrix[0][j].bottom_right_db_guid);
    }
    ...
    for ( i = 1; i < n_tiles_height + 1; ++i ) {
        ...
        ocrDbCreate( &tile_matrix[i][0].right_column_db_guid, &db_guid_i_0_rc_data, sizeof(int)*tile_height, FLAGS, NULL, NO_ALLOC );
        assert(db_guid_i_0_rc_data);
        allocated = (int*)db_guid_i_0_rc_data;
        for ( j = 0; j < tile_height ; ++j ) {
            allocated[j] = GAP*((i-1)*tile_height+j+1);
        }
        ocrEventSatisfy(tile_matrix[i][0].right_column_event_guid, tile_matrix[i][0].right_column_db_guid);

        void* db_guid_i_0_br_data;
        ocrDbCreate( &tile_matrix[i][0].bottom_right_db_guid, &db_guid_i_0_br_data, sizeof(int), FLAGS, NULL, NO_ALLOC );
        ...
        ocrEventSatisfy(tile_matrix[i][0].bottom_right_event_guid, tile_matrix[i][0].bottom_right_db_guid);
    }
}
...
void freeTileMatrix ( Tile_t** tile_matrix, int n_tiles_height, int n_tiles_width ) {
    ...
    for ( i = 0; i < n_tiles_height+1; ++i ) {
        for ( j = 0; j < n_tiles_width+1; ++j ) {
            ...
            if ( NULL_GUID != tile_matrix[i][j].bottom_row_db_guid) {
                ocrDbDestroy(tile_matrix[i][j].bottom_row_db_guid);
            }
            if ( NULL_GUID != tile_matrix[i][j].right_column_db_guid) {
                ocrDbDestroy(tile_matrix[i][j].right_column_db_guid);
            }
            if ( NULL_GUID != tile_matrix[i][j].bottom_right_db_guid) {
                ocrDbDestroy(tile_matrix[i][j].bottom_right_db_guid);
            }
        }
        free(tile_matrix[i]);
    }
    ...
    for ( times = 0; times < nTimes; ++times ) {
        ...
        for ( i = 0; i < n_tiles_height+1; ++i ) {
            ...
            for ( j = 0; j < n_tiles_width+1; ++j ) {
                ...
                tile_matrix[i][j].bottom_row_db_guid = NULL_GUID;
                tile_matrix[i][j].right_column_db_guid = NULL_GUID;
                tile_matrix[i][j].bottom_right_db_guid = NULL_GUID;
            }
        }
