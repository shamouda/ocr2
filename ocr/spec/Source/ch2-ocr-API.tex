...
a succesfull call, the runtime will return the GUID for the newly
...
%%%%
\descr
This API is used to create the basic unit of data in OCR: the
data-block. Unless DB_PROP_NO_ACQUIRE is specified in \em{flags}, this
API call also acquires the newly created data-block and returns a
pointer to the memory region (in \em{addr}).

The created data-block:
\begin{DoxyItemize}
\item Will always be 8-byte aligned (ie: if \em{addr} is returned, it
...
\end{DoxyItemize}
If a valid address is returned in \em{addr}, it will point to the
first byte of the data-block and the user can safely read or write
to a range of length \emph{len} starting at \em{addr} until the end of
the EDT or a call to \bf{ocrDbRelease()} on this data-block, whichever
comes first.

\begin{DoxyNote}{Note}
\begin{DoxyItemize}
\item Using the DB_PROP_NO_ACQUIRE flag is recommended to allow the
  runtime to make placement decisions for newly created
  data-blocks. Not specifiying this flag may result in a sub-optimal
  memory placement for the created data-block.
\item When DB_PROP_NO_ACQUIRE is specified, a conformant
...
\item Like all GUIDs, the uniqueness of a data-block's GUID is not
  ...
  (between \bf{ocrDbCreate()} and \bf{ocrDbDestroy()}).
\end{DoxyItemize}
\end{DoxyNote}
