...
\returns
 It returns a
\code{ocrGuid\_t} value for compatibility purposes, but is ignored by the runtime.
...
{ocr\-Shutdown} (void)
\begin{DoxyCompactList}\small\item\em Called by an E\-D\-T to indicate the normal end of an O\-C\-R program. \end{DoxyCompactList}
...
Called by an E\-D\-\T to indicate the normal end of the program and allows for the termination of the runtime.
Program behavior after this call is undefined, hence all program functionality should be completed prior to calling this function.
...
Program behavior after this call is undefined. The error code for the reason for abort is passed to the runtime, and its handling is implementation dependent.
...
The single parameter for this function is the pointer to the sole datablock passed to main\-Edt from the runtime.
...
\item \hypertarget{type_ocrInDbAllocator_t}\code{enum ocrInDbAllocator\_t} containing:
  ...
  \end{itemize}
\item \code{enum ocrDbAccessMode\_t} containing the four access modes:
  \begin{itemize}
    \item \code{DB\_MODE\_RW}
    \item \code{DB\_MODE\_EW}
    \item \code{DB\_MODE\_RO}
    \item \ocde{DB\_MODE\_CONST}
  \end{itemize}
