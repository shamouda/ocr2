...
\item \code{EDT\_PARAM\_UNK} which specifies that the number of
  parameters or dependences to an EDT or EDT template is unknown at this time
...
of all EDTs created based on the template. This function creats an EDT template.
...
this template will take. If EDTs created based on this template can take a varaible
number of arguments, the constant EDT\_PARAM\_UNK can be used.\\
\hline
\end{DoxyParams}
...
\begin{DoxyItemize}
\item 0: Successful
\item OCR\_ENOMEM: The runtime could not allocate the metadata
  for the template
\end{DoxyItemize}
...
mode. If you only want to specify only some of the pre-slots use
\code{UNINITIALIZED\_GUID} for the unkown ones.\\
\hline
...
\begin{DoxyItemize}
\item 0: Successful
\item OCR\_ENOMEM: The runtime could not create the metadata required for the
EDT
\item OCR\_EINVAL: The GUID specifying the template is invalid or the number
  of parameters or pre-slots is not fully resolved
\end{DoxyItemize}
...
If the EDT had an associated \code{outputEvent}, that event will also be explictly
destroyed.
\end{DoxyNote}
...
\item Event to event: The destination event's pre-slot will become satisfied
  ...
  This allows the formation of event chains.
...
\item Data block to event: Equivalent to \hyperlink{func_ocrEventSatisfySlot}{
  \code{ocrEventSatisfySlot}}:\code{ocrAddDependence(db, evt, slot)} is
...
\item NULL\_GUID to event or EDT: This is equivalent to ``data block to event''
  or ``data block to EDT'' where the data is non-existant.
\end{DoxyItemize}
