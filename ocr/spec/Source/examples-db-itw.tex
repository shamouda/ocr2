%%%%
This example illustrates the usage model for datablocks accessed with the {\tt Intent-To-Write (ITW)} mode.
The {\tt ITW} mode ensures that only one master copy of the datablock exists at any time inside a shared address space.
...
separate memory regions of the same datablock with the {\tt ITW} mode.
