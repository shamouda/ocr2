...
% Vincent writes:
%
%   I think the graph misses direct transitions from certain states to
%  destroyed. Basically when calling ocrEdtDestroy in “available” state
%  and eventually, in “resolved” state. The user respectively knows not
%   all dependences will be added and not all dependences will be satisfied
%  ever. It’s a limited way for the user to “cancel” an edt. If we do that then
%  we need to define when the “reference count” for an EDT is set to 1 so
%  that subsequent ocrEdtDestroy will have to wait for the edt to complete
%  execution. Future specification/apis could also introduce cancellation
%  arrows from other state.
%
%    .... we need to think about his comments and update the following figure accordingly
...
%  Vincent writes (pertaining to the following paragraph) ...
%
% "Typically, the EDT that terminates the program is the last EDT in the
%  program DAG, and the programmer has assured that all other EDTs in the
%  DAG have completed execution before the function to terminate the program is called.”
%  => That goes back to my comment on page 12 (the figure with teh execution states of an EDT).
%   I think it depends how you define “completed execution” or for which EDT state one
% should assume the EDT “has to necessarily complete" at some point. For instance, I don’t
%   see a correctness issue calling shutdown while some edts are available/resolved but will
%  never transition to their next state because the programer ensures it. Am I presuming too
%   much because I know the implementation or does the spec defined as it is allowed me that
