...
#define STATS_SETUP(PROCESS, GUID)                      \
    do {                                                \
        ...
        DPRINTF(DEBUG_LVL_INFO, "Created a StatsProcess (0x%lx) for GUID "GUIDSx"\n", (u64)PROCESS, GUIDFS(GUID)); \
    } while(0);

#define STATS_DESTRUCT(PROCESS)                         \
    do {                                                \
        u64 i, j;                                       \
        DPRINTF(DEBUG_LVL_INFO, "Destroying StatsProcess 0x%lx (GUID: "GUIDSx")\n", (u64)PROCESS, GUIDFS(PROCESS->me)); \
