...
def appendPreHTML(outFile, exeTime, numThreads, numEDTs, totalTime):
    ...
    outFile.write('\t\t}\n')
    outFile.write('\t</style>\n\n')
    outFile.write('\t<script src="' + JAVASCRIPT_URL + '"\n')
    outFile.write('\t\t\tonerror="document.write(\'Cannot display timeline...<BR><BR> Unable to load visJs from Xstack server. Make sure you have internet connectivity. To view offline you must have visJs on your local machine.  VisJs can be found here: http://visjs.org/#download_install\');"></script>\n')
