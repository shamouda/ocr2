...
parser.add_argument('--binding', dest='binding', default='0',
                   help='thread binding to use (default: no binding)')
parser.add_argument('--alloc', dest='alloc', default='32',
                   help='size (in MB) of memory available for app use (default: 32)')
parser.add_argument('--alloctype', dest='alloctype', default='mallocproxy',
...
def GenerateComp(output, threads, binding):
...
def GenerateConfig(filehandle, guid, platform, target, threads, binding, alloc, alloctype):
    filehandle.write("# Generated by python script, modify as you see fit\n\n")
    GenerateGuid(filehandle, guid)
    if binding != '0':
        bound_threads = binding.count(',')+1
        if bound_threads != threads:
            print 'Error: ', threads, ' threads given, however binding is for ', bound_threads
            raise
    if target=='X86':
        ...
        GenerateComp(filehandle, threads, binding)
    elif target=='FSIM':
...
if platform=='X86' or platform=='FSIM':
    ...
    if os.path.isfile(outputfilename):
        ...
        with open(outputfilename, "w") as filehandle:
            GenerateConfig(filehandle, guid, platform, target, threads, binding, alloc, alloctype)
else:
