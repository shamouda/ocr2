...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrSetHintValue( & dbHint, OCR_HINT_DB_AFFINITY, affinity );

    // Create DB @ affinity
    void * dbPtr;
    ocrGuid_t dbGuid;
    u32 nbElem = NB_ELEM_DB;
    ocrDbCreate(&dbGuid, &dbPtr, sizeof(TYPE_ELEM_DB) * NB_ELEM_DB, 0, PICK_1_1(&dbHint,affinity), NO_ALLOC);
    u64 i = 0;
    int * data = (int *) dbPtr;
    while (i < nbElem) {
        data[i] = -1;
    ...
    ocrSetHintValue( & edtHint, OCR_HINT_EDT_AFFINITY, affinity );
    ...
    ocrEdtCreate(&shutdownGuid, shutdownEdtTemplateGuid, 0, NULL, NB_WRITERS+1, NULL,
                 EDT_PROP_NONE, PICK_1_1(&edtHint,affinity), NULL);
    ...
    while (i < NB_WRITERS) {
        ...
        ocrHintInit( &edtHint, OCR_HINT_EDT_T );
        ocrSetHintValue( & edtHint, OCR_HINT_EDT_AFFINITY, affinities[i % affinityCount] );
        ocrEdtCreate(&writerEdtGuid, writerEdtTemplateGuid, 1, &i, 1, NULL,
                     EDT_PROP_NONE, PICK_1_1(&edtHint,affinities[i % affinityCount]), &outputEventGuid);
