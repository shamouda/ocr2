...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    while (db < NB_DBS) {
        ...
        ocrSetHintValue( & dbHint, OCR_HINT_DB_AFFINITY, affinities[db%affinityCount]);
        ocrDbCreate(&dbGuid[db], &dbPtr, sizeof(TYPE_ELEM_DB) * NB_ELEM_DB, 0, PICK_1_1(&dbHint,affinities[db%affinityCount]), NO_ALLOC);
        u64 i = 0;
        int * data = (int *) dbPtr;
        while (i < nbElem) {
            data[i] = -1;
