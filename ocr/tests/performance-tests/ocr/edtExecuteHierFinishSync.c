    ...
    if (depth == TREE_DEPTH) {
        ...
        while (i < LEAF_FANOUT) {
            ...
            ocrEdtCreate(&workEdtGuid, workEdtTemplateGuid,
                         0, NULL, 0, NULL_GUID, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
            i++;
        }
        ...
        while (i < NODE_FANOUT) {
            ...
            ocrEdtCreate(&spawnerEdtGuid, spawnerEdtTemplateGuid,
                         1, &depth, 0, NULL_GUID, EDT_SPAWN_PROPS, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
            i++;
        }
...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrEdtCreate(&terminateEdtGuid, terminateEdtTemplateGuid,
                 0, NULL, 2, NULL_GUID, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);

    timestamp_t * dbPtr;
    ocrGuid_t dbGuid;
    ocrDbCreate(&dbGuid, (void **)&dbPtr, (sizeof(timestamp_t)*2), 0, PICK_1_1(NULL_HINT,NULL_GUID), NO_ALLOC);
    ocrDbRelease(dbGuid);
    ...
    ocrEdtCreate(&finishEdtGuid, finishEdtTemplateGuid,
                 0, NULL, 1, NULL_GUID,  EDT_PROP_FINISH, PICK_1_1(NULL_HINT,NULL_GUID), &oEvtGuid);
