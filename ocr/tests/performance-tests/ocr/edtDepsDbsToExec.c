...
ocrGuid_t edtCode(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrEdtCreate(&edtDriverGuid, edtDriverTemplateGuid,
                 2, paramvDriverEdt, 2, NULL, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
...
ocrGuid_t driverEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    if (driverIteration == NB_ITERS) {
        ...
        ocrEdtCreate(&edtGuid, edtCodeTemplateGuid,
                     3, paramvEdtCode, DB_NBS+1, NULL_GUID, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrDbCreate(&dbAllGuids, (void **)&dbAllPtr, (sizeof(ocrGuid_t)*DB_NBS), 0, PICK_1_1(NULL_HINT,NULL_GUID), NO_ALLOC);

    // Create all DBs and record their guid in dbAllPtr
    int i = 0;
    while (i < DB_NBS) {
        DB_TYPE * dbPtr;
        ocrDbCreate(&dbAllPtr[i], (void **)&dbPtr, (sizeof(DB_TYPE)*DB_NB_ELT), 0, PICK_1_1(NULL_HINT,NULL_GUID), NO_ALLOC);
    ...
    ocrDbCreate(&dbTimerGuid, (void **)&dbTimerPtr, sizeof(timestamp_t)*2, 0, PICK_1_1(NULL_HINT,NULL_GUID), NO_ALLOC);
    ocrDbRelease(dbTimerGuid);
    ...
    ocrEdtCreate(&driverEdtGuid, edtDriverTemplateGuid,
                 2, paramvDriverEdt, 2, NULL_GUID, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
