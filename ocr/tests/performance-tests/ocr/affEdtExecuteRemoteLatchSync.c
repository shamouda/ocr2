...
ocrGuid_t headEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrSetHintValue( & edtHint, OCR_HINT_EDT_AFFINITY, affinities[count-1] );
    ...
    while (k < NB_INSTANCES) {
        ...
        ocrEdtCreate(&workEdtGuid, workEdtTemplateGuid,
                     1, (u64 *) &evtGuid, 0, NULL_GUID, EDT_PROP_NONE, PICK_1_1(&edtHint, affinities[count-1]), NULL);
        get_time(&ts_stop);
...
ocrGuid_t lchCreatorEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    *guid = evGuid;
    ...
    ocrEdtCreate(&terminateEdtGuid, terminateEdtTemplateGuid,
                 0, NULL, 1, &evGuid, EDT_PROP_NONE, PICK_1_1(NULL_HINT,NULL_GUID), NULL);
    ...
    ocrEdtCreate(&headEdtGuid, headEdtTemplateGuid,
                 0, NULL, 1, &(depv[0].guid), EDT_PROP_NONE, PICK_1_1(&edtHint,affinities[0]), NULL);
...
ocrGuid_t mainEdt(u32 paramc, u64* paramv, u32 depc, ocrEdtDep_t depv[]) {
    ...
    ocrGuid_t * dbLchPtr;
    ocrGuid_t dbLchGuid;
    ocrDbCreate(&dbLchGuid, (void **)&dbLchPtr, sizeof(ocrGuid_t), 0, PICK_1_1(NULL_HINT,NULL_GUID), NO_ALLOC);
    ...
    ocrEdtCreate(&lchCreatorEdtGuid, lchCreatorTemplateGuid,
                 0, NULL, 1, &dbLchGuid, EDT_PROP_NONE, PICK_1_1(&edtHint,affinities[count-1]), NULL);
    return NULL_GUID;
}
